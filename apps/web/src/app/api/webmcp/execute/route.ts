  ...
  if (isExplicitAgent || (!bearerToken && req.nextUrl.searchParams.get('simulate_agent') === 'true')) {
    return {
      ...
      errorResponse: {
        ...
        authorization_url: `https://app.netify.ng/oauth/authorize?client_id=chatgpt-agent&response_type=code&scope=receivables:read%20customers:read%20customer_evidence:read%20business_memory:read%20collection_messages:draft&code_challenge=E9Melhoa2OwvFrEMTJguCHaoeK1t8URWbuGJSstw-cM&code_challenge_method=S256`,
        required_scope: requiredScope,
        tool,
...
async function executeToolLogic(
  ...
  input: any,
  token: string,
  authEval: { agentPayload?: AgentAccessTokenPayload }
) {
  // Helper to resolve an open receivable for a customer
  async function getCustomerReceivableId(customerId: string): Promise<string | undefined> {
    try {
      const recsRes = await fetch(`${API_BASE_URL}/receivables?customerId=${customerId}&status=OPEN`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const recsData = await recsRes.json();
      if (recsData?.data?.[0]?.id) return recsData.data[0].id;

      const anyRecsRes = await fetch(`${API_BASE_URL}/receivables?customerId=${customerId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const anyRecsData = await anyRecsRes.json();
      return anyRecsData?.data?.[0]?.id;
    } catch {
      return undefined;
    }
  }
  ...
  if (tool === 'list_receivables') {
    const isOverdue = input.isOverdue === true || input.isOverdue === 'true';
  ...
  if (tool === 'get_customer_evidence') {
    ...
    const [custRes, recsRes, commsRes, memoriesRes] = await Promise.all([
      ...
      fetch(`${API_BASE_URL}/customers/${customerId}/commitments`, { headers: { Authorization: `Bearer ${token}` } }),
      fetch(`${API_BASE_URL}/customers/${customerId}/memories`, { headers: { Authorization: `Bearer ${token}` } }),
    ]);
    ...
    return jsonResponse({
      ...
      businessMemories: memories?.items || memories?.data,
    });
  }
  ...
  if (tool === 'get_customer_risk_profile') {
    const customerId = input.customerId || 'f14e802a-573d-46bb-8257-317bdc3cddb0';
    const apiRes = await fetch(`${API_BASE_URL}/risk/customer/${customerId}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const result = await apiRes.json();
    return jsonResponse({
  ...
  if (tool === 'get_daily_briefing') {
    ...
    const apiRes = await fetch(`${API_BASE_URL}/command-center/briefing?${queryParams.toString()}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const result = await apiRes.json();
    return jsonResponse({
      ...
      briefing: result?.data?.briefing || result?.data,
  ...
  if (tool === 'list_notifications') {
    const unreadOnly = input.unreadOnly === true || input.unreadOnly === 'true';
  ...
  if (tool === 'query_business_memory') {
    const customerId = input.customerId || 'f14e802a-573d-46bb-8257-317bdc3cddb0';
    const apiRes = await fetch(`${API_BASE_URL}/customers/${customerId}/memories`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const result = await apiRes.json();
    return jsonResponse({
      ...
      memories: result?.items || result?.data,
      tenantIsolated: true,
    });
  }
  ...
  if (tool === 'draft_follow_up_message') {
    ...
    try {
      const apiRes = await fetch(`${API_BASE_URL}/ai/customers/${customerId}/draft-message`, {
        method: 'POST',
        headers: {
        ...
        signal: AbortSignal.timeout(12000), // 12s safeguard against upstream LLM latency
      });
      const result = await apiRes.json();
      return jsonResponse({
        success: true,
        tool: 'draft_follow_up_message',
        draft: result?.data?.messageBody || result?.data?.message || result?.data?.draft || 'Good day. Respectful reminder on your overdue invoice balance with Netify. Please advise your payment timeline.',
        recipientName: result?.data?.recipientName,
        recipientContact: result?.data?.recipientContact,
        verifiedOutstandingAmount: result?.data?.verifiedOutstandingAmount,
      ...
      return jsonResponse({
        success: true,
        tool: 'draft_follow_up_message',
        draft: 'Good day. Respectful reminder on your overdue invoice balance with Netify. Please advise your payment timeline.',
        channel,
        tone,
        safeguard: 'DRAFT_ONLY_NO_EXTERNAL_DISPATCH',
      });
  ...
  // 10. create_payment_commitment (Consequential write with Human-in-the-loop Safeguard)
  if (tool === 'create_payment_commitment') {
    ...
    // Resolve receivableId if not provided
    let receivableId = input.receivableId;
    if (!receivableId) {
      receivableId = await getCustomerReceivableId(customerId);
    }

    if (!receivableId) {
      return jsonResponse({
        success: false,
        error: 'No active receivable found for target customer to attach commitment to.',
      }, { status: 400 });
    }
  ...
  // 11. record_collection_activity (Consequential write with Human-in-the-loop Safeguard)
  if (tool === 'record_collection_activity') {
    ...
    if (!receivableId) {
      return jsonResponse({
        success: false,
        error: 'No active receivable found for target customer to attach collection activity to.',
      }, { status: 400 });
    }
  ...
  return jsonResponse({
    success: true,
    tool,
    message: `Tool "${tool}" executed successfully via WebMCP Gateway.`,
  });
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    searchParams.forEach((val, key) => {
      if (key !== 'tool' && key !== 'access_token' && key !== 'token') {
        input[key] = val;
      }
    });

    return await executeToolLogic(tool, input, authEval.token, authEval);
  } catch (err: any) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const rawTool = body.tool || req.nextUrl.searchParams.get('tool') || 'get_collection_priority';
    const tool = normalizeToolName(rawTool);
